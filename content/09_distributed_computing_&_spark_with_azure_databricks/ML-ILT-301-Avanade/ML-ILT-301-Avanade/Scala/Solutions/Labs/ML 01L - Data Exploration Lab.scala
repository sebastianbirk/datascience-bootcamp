...
println("We have %d records that contain null values.".format(recordCount - noNullsRecordCount))
println("*-"*80)
...
val avgPrice = airbnbDF.select(avg("price")).first().getDouble(0)
val medianPrice = airbnbDF.stat.approxQuantile("price", probabilities=Array(0.5), relativeError=0.01)(0)
...
// MAGIC 
// MAGIC We are going to use SparkML's `RegressionEvaluator` to compute the RMSE for our average price and median price predictions [Python](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.evaluation.RegressionEvaluator)/[Scala](https://spark.apache.org/docs/latest/api/scala/#org.apache.spark.ml.evaluation.RegressionEvaluator).
...
println (s"The RMSE for predicting the average price is: ${regressionMeanEvaluator.evaluate(predDF)}")

val regressionMedianEvaluator = new RegressionEvaluator()
  .setPredictionCol("medianPrediction")
  .setLabelCol("price")
  .setMetricName("rmse")

println (s"The RMSE for predicting the median price is: ${regressionMedianEvaluator.evaluate(predDF)}")
println("*-"*80)
